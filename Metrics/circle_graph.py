...
bag = rosbag.Bag('hector_orbslam3_circle.bag') 
...
for topic, msg, t in bag.read_messages(topics=['/slam_out_pose','/odom','/orb_slam3/camera_pose']):
    ...
    if topic == '/slam_out_pose':
        ...
        scale_axle_y=1
        scale_axle_z=1
        x1=(1*scale_axle_x*msg.pose.position.z)
        y1=(-1*scale_axle_y*msg.pose.position.x)
...
ax.set_xlim(-2.5,2.5)   # Range and scale for x-axis
ax.set_ylim(-2.5,2.5)  # Range and scale for the y-axis

x1, x2, y1, y2 = -0.3, -0.1, 0.37, 0.47   # subregion of the original image
#ODOM 
...
axins = ax.inset_axes(
    [0.6, 0.6, 0.3, 0.3],xlim=(x1, x2), ylim=(y1, y2), xticklabels=[], yticklabels=[])
